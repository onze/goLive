...
class Unit(Entity):
   ...
   def add_to_path(self,tile):
      ...
      '''
      if tile.eid==self.current_tile.eid:
         #already done by nature
         return
   ...
   def update_path_following(self):
      ...
      if len(self.path)==0:
         out('WARNING in Unit.update_path_following: self.path==0, but method still in update_list. removing it.')
